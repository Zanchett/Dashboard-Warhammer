...
export async function removeUpgradeFromUser(username: string, category: string, upgradeName: string): Promise<{ success: boolean; message?: string }> {
...
export async function cleanUpgradesDatabase(): Promise<{ success: boolean; message: string }> {
  try {
    ...
    return { 
      success: false, 
      message: error instanceof Error ? error.message : "An unexpected error occurred while cleaning the upgrades database"
    };
  }
}
